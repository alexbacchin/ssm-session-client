...
func init() {
	...
	ssmSshDirectCmd.Flags().StringVar(&config.Flags().SSHDirect.SSHKeyFile, "ssh-key", "", "Path to SSH private key file (default: auto-discover)")
	ssmSshDirectCmd.Flags().BoolVar(&config.Flags().SSHDirect.NoHostKeyCheck, "no-host-key-check", false, "Skip host key verification (warning: disables MITM protection)")
	...
	viper.BindPFlag("ssh-direct.no-host-key-check", ssmSshDirectCmd.Flags().Lookup("no-host-key-check"))     //nolint:errcheck
